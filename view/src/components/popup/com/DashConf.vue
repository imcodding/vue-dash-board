<template>
  <div class="popBox" >
    ...
    <div class="mt5">
      <grid ref="img" :id="'imgGrid'" style="max-height: 330px;overflow: auto;"></grid>
    </div>
...
export default {
  ...
  mounted() {
    this.imgSrc = process.env.NODE_ENV == 'production' ? '/image/vue/bg/' : '/static/img/d3/bg/'
    let src = this.imgSrc;
    this.$hmGrid.create(this.$refs.img.$refs.imgGrid, {
          columns: [
              { text: '이미지명', datafield: 'imgName', minwidth: 100, editable: false },
              { text: '이미지', datafield: 'imgUid', width: 102, cellsalign: 'center', editable: false,
                cellsrenderer: function(row, datafield, value) {
                  var cell = '<div style="margin: 5px;" class="jqx-center-align">';
                      cell += '<img height="50" width="80" src="' + src + '{0}.png" />'.substitute(value || 'NoImage');
                      cell += '</div>';
                      return cell;
                  }
              },
              // { text: '기본이미지', datafield: 'disSConf', width: 100, columntype: 'checkbox', editable: false },
              // { text: '표시순서', datafield: 'sortIdx', width: 100, columntype: 'numberinput', cellsalign: 'right' },
              // { text: '표시여부', datafield: 'isDisplay', width: 100, columntype: 'checkbox' },
          ],
          height: '320px',
          rowsheight: 60,
      })
  ...
  methods: {
    ...
    searchImg() {
      ...
      this.$hmGrid.refreshData(this.$refs.img.$refs.imgGrid, '/api/dash/setting/imgList', {imgKind2:'BG', mode: mode, searchText: this.searchText})
    },
    applyImg() {
      ...
      this.$store.dispatch('updateImgPath', {img: row.imgUid, imgKind2: 'BG'})
    },
    addImg() {
      this.popupInfo.name = 'com/addImg'
      this.popupInfo.title = '배경 이미지 등록'
      this.popupInfo.width = 400
      this.popupInfo.height = 193
      this.popupInfo.params = {imgKind2:'BG'}
      this.modal();
    },
    deleteImg() {
      let selectedIdxes = this.$refs.img.$refs.imgGrid.getselectedrowindexes();
