...
import TopoUtil from './topo.util';
...
var ifIdx1_Poll = null, ifIdx2_Poll = null;
var valueTop = 0
...
var TopoLink = {
		...
		setLinkData: function(list) {
			...
			for(var i = 0; i < list.length; i++) {
				...
				if(document.getElementById('#g' + d.itemNo1) != null || document.getElementById('#g' + d.itemNo2) != null) {
					continue;
				}
				// 1,3 있는 경우 고려해서 변경
				if(d.point1==0&&d.point2==0&&d.point3==0){
                    links.push({...d, source: "g" + d.itemNo1, target: "g" + d.itemNo2, itemType: 'link'});
				}else if(d.point1!=0&&d.point2==0&&d.point3==0){
                    links.push({...d, source: "g" + d.itemNo1, target: "g" + d.point1, itemType: 'link'});
                    links.push({...d, source: "g" + d.point1, target: "g" + d.itemNo2, itemType: 'link'});
                }else if(d.point1!=0&&d.point2!=0&&d.point3==0){
                    links.push({...d, source: "g" + d.itemNo1, target: "g" + d.point1, itemType: 'link'});
                    links.push({...d, source: "g" + d.point1, target: "g" + d.point2, itemType: 'link'});
                    links.push({...d, source: "g" + d.point2, target: "g" + d.itemNo2, itemType: 'link'});
                }else if(d.point1!=0&&d.point2!=0&&d.point3!=0){
                    links.push({...d, source: "g" + d.itemNo1, target: "g" + d.point1, itemType: 'link'});
                    links.push({...d, source: "g" + d.point1, target: "g" + d.point2, itemType: 'link'});
                    links.push({...d, source: "g" + d.point2, target: "g" + d.point3, itemType: 'link'});
                    links.push({...d, source: "g" + d.point3, target: "g" + d.itemNo2, itemType: 'link'});
                }else if(d.point1==0&&d.point2!=0&&d.point3==0){
                    links.push({...d, source: "g" + d.itemNo1, target: "g" + d.point2, itemType: 'link'});
                    links.push({...d, source: "g" + d.point2, target: "g" + d.itemNo2, itemType: 'link'});
                }else if(d.point1==0&&d.point2!=0&&d.point3!=0){
                    links.push({...d, source: "g" + d.itemNo1, target: "g" + d.point2, itemType: 'link'});
                    links.push({...d, source: "g" + d.point2, target: "g" + d.point3, itemType: 'link'});
                    ...
                    links.push({...d, source: "g" + d.itemNo1, target: "g" + d.point3, itemType: 'link'});
                    links.push({...d, source: "g" + d.point3, target: "g" + d.itemNo2, itemType: 'link'});
                }else if(d.point1!=0&&d.point2==0&&d.point3!=0){
                    links.push({...d, source: "g" + d.itemNo1, target: "g" + d.point1, itemType: 'link'});
                    links.push({...d, source: "g" + d.point1, target: "g" + d.point3, itemType: 'link'});
                    links.push({...d, source: "g" + d.point3, target: "g" + d.itemNo2, itemType: 'link'});
                }
			}
			return links;
		...
		callRefresh: function(objTopo) {
			this.refresh(objTopo, objTopo.vars.svg, objTopo.vars.links);
		},
        ...
        link = link.enter()
            ...
            .style("stroke", function (d) {
                 ...
                 */
                    ...
                    ifIdx1_Poll = d.ifIdx1 > 0 ? 'Y' : 'N';
                    ifIdx2_Poll = d.ifIdx2 > 0 ? 'Y' : 'N';

                    if (ifIdx1_Poll === 'Y') color1 = d.pollingColor || TopoConst.envSetting.pollingColor;
                    else color1 = d.lineColor || TopoConst.envSetting.lineColor;

                    if (ifIdx2_Poll === 'Y') color2 = d.pollingColor || TopoConst.envSetting.pollingColor;
                    else color2 = d.lineColor || TopoConst.envSetting.lineColor;
    ...
    getPathLink: function(linkNo, objTopo) {
    ...
    addEvent: function (link, objTopo) {
        ...
        link.on("contextmenu", d3.contextMenu(objTopo.clickMenu, null, objTopo))
            .on("click", function (d) {
                ...
                toolTipLink = TopoLink.getPathLink(d.linkNo,objTopo);
                toolTipLink
            ...
            .on("mousemove", function (d) {
                TopoLink.moveTooltip(d, objTopo);
            })
    ...
    showTooltip: function (d, objTopo) {
      console.log('zz', d)
        ...
        div = d3.select('div#section').select('div.topoTooltip');
        _devKind1 = d.hasOwnProperty("itemType") ? d.itemType.toUpperCase() : null;
        if (_devKind1 == null || _devKind1 != "LINK") return;


        tHtml = '';
        tHtml += "<table class='tt-table'><thead>";
        tHtml += "<tr class='tt-tr'><th colspan='2'>" + "[{0}] 회선 정보".substitute(d_source.itemName || "") + "</th></tr>";
        tHtml += "</thead>";
        tHtml += "<tbody>";
        if (d.linkName1 != null && d.linkName1 != "") {
            tHtml += "<tr class='tt-tr'><th class='tt-th'>" + "회선명 : " + "</th>";
            tHtml += "<td class='tt-td'>" + "{0}".substitute(d.linkName1) + "</td></tr>";
            tHtml += "<tr class='tt-tr'><th class='tt-th'>" + "대역폭 : " + "</th>";
            tHtml += "<td class='tt-td'>" + "{0}".substitute(HmUtil.convertUnit1000(d.lineWidth1)) + "</td></tr>";
            if (d.isMulti == 1) {
                tHtml += "<tr class='tt-tr'><th class='tt-th'>" + "트래픽 : " + "</th>";
                tHtml += "<td class='tt-td'>" + "{0}bps ({1})".substitute(HmUtil.convertUnit1000(d.traffic1), TopoConst.trfTypeString[d.trafficType1]) + "</td></tr>";
            }
            else {
                tHtml += "<tr class='tt-tr'><th class='tt-th'>" + "트래픽 : " + "</th>";
                tHtml += "<td class='tt-td'>" + "{0} (In) / {1} (Out)".substitute(HmUtil.convertUnit1000(d.traffic1In), HmUtil.convertUnit1000(d.traffic1Out)) + "</td></tr>";
            }
        }
        tHtml += "<tr class='tt-tr'><th class='tt-th'>" + "상태 : " + "</th>";
        tHtml += "<td class='tt-td'>" + "{0}".substitute(TopoUtil.convertEvtLevel(d.evtLevel1)) + "</td></tr>";
        tHtml += "</tbody></table><br>";

        tHtml += "<table class='tt-table'><thead>";
        tHtml += "<tr class='tt-tr'><th colspan='2'>" + "[{0}] 회선 정보".substitute(d_target.itemName || "") + "</th></tr>";
        tHtml += "</thead>";
        tHtml += "<tbody>";
        if (d.linkName2 != null && d.linkName2 != "") {
            tHtml += "<tr class='tt-tr'><th class='tt-th'>" + "회선명 : " + "</th>";
            tHtml += "<td class='tt-td'>" + "{0}".substitute(d.linkName2) + "</td></tr>";

            tHtml += "<tr class='tt-tr'><th class='tt-th'>" + "대역폭 : " + "</th>";
            tHtml += "<td class='tt-td'>" + "{0}".substitute(HmUtil.convertUnit1000(d.lineWidth2)) + "</td></tr>";
            if (d.isMulti == 1) {
                tHtml += "<tr class='tt-tr'><th class='tt-th'>" + "트래픽 : " + "</th>";
                tHtml += "<td class='tt-td'>" + "{0}bps ({1})".substitute(HmUtil.convertUnit1000(d.traffic2), TopoConst.trfTypeString[d.trafficType2]) + "</td></tr>";
            }
            else {
                tHtml += "<tr class='tt-tr'><th class='tt-th'>" + "트래픽 : " + "</th>";
                tHtml += "<td class='tt-td'>" + "{0} (In) / {1} (Out)".substitute(HmUtil.convertUnit1000(d.traffic2In), HmUtil.convertUnit1000(d.traffic2Out)) + "</td></tr>";
            }
        }
        tHtml += "<tr class='tt-tr'><th class='tt-th'>" + "상태 : " + "</th>";
        tHtml += "<td class='tt-td'>" + "{0}".substitute(TopoUtil.convertEvtLevel(d.evtLevel2)) + "</td></tr>";
        tHtml += "</tbody></table>";
        show(tHtml);
        ...
        function show(msg) {
            ...
            div.html(msg)
                .style("left", function (d) {
                    ...
                    _left = d3.event.layerX;
                    return _left + 10 + "px";
                })
                .style("top", function (d) {
                    tooltipSize.height = $('.topoTooltip').height() + 40;
                    _top = d3.event.offsetY + tooltipSize.height >= browserSize.height ? d3.event.offsetY - tooltipSize.height - 30 : d3.event.clientY;
                    valueTop = 10;// _top = d3.event.clientY + tooltipSize.height > browserSize.height ? (browserSize.height - tooltipSize.height) - (tooltipSize.height / 2) : d3.event.clientY;
    ...
    moveTooltip: function (d, objTopo) {
        ...
        div.style("left", function(d) {
            tooltipSize.width = $('.topoTooltip').width() + 30;
            _left = d3.event.offsetX + tooltipSize.width >= browserSize.width ? d3.event.offsetX - (tooltipSize.width ) : d3.event.offsetX;
            return _left + 10 + "px";
        }).style("top", function (d){
            tooltipSize.height = $('.topoTooltip').height() + 40;
            _top = d3.event.offsetY + tooltipSize.height >= browserSize.height ? d3.event.offsetY - tooltipSize.height - 30 : d3.event.offsetY;
            valueTop = 10;//$('#nav').height() ? $('#nav').height() : $('#hd-header').hetopoTooltipight();
            return _top + valueTop  + "px";
        });
    },
